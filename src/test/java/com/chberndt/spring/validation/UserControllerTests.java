...
public class UserControllerTests {
  ...
  public void shouldNotCreateUserWithInvalidFields() throws Exception {
    ...
    assertThat((Integer) documentContext.read("$.errors.length()")).isEqualTo(2);

    assertThat(
        documentContext.read("$.errors[?(@.field == 'email')].defaultMessage", JSONArray.class)
            .get(0)).isEqualTo("must be a well-formed email address");

    assertThat(
        documentContext.read("$.errors[?(@.field == 'userName')].defaultMessage", JSONArray.class)
            .get(0)).isEqualTo("must match \"^[a-zA-Z0-9]*$\"");
  }
  ...
  public void shouldNotCreateUserWithoutRequiredFields() {
    ...
    assertThat(
        documentContext.read("$.errors[?(@.field == 'email')].defaultMessage", JSONArray.class)
            .get(0)).isEqualTo("must not be empty");
